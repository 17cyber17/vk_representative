...
- Сервисный ключ доступа VK (service key)

## Как получить сервисный ключ доступа VK

1. Зарегистрируйте приложение в VK: <https://vk.com/apps?act=manage>.
2. Откройте настройки приложения и скопируйте **Сервисный ключ доступа**.
3. Убедитесь, что у приложения есть доступ к нужной стене (обычно для публичных сообществ).
...
| Переменная | Описание |
| --- | --- |
| `VK_SERVICE_KEY` | Сервисный ключ доступа VK |
